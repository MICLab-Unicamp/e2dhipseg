...
def get_largest_components(volume, mask_ths=0.5, ncomponents=2, debug=False):
    ...
    if hlabel_len >= 2:
        return (np.ma.masked_not_equal(labels, highest_labels[0]).filled(0) + np.ma.masked_not_equal(labels, highest_labels[1]).filled(0)).astype(np.bool).astype(np.float32)
    else:
...
def ialabel_test():
    ...
    while True:
        ...
        cv.imshow("5 Biggest Components", cv.resize(np.hstack((gimg, timg, labeled, (labeledint == biggest_label).astype(np.float32))), (1920, 600)))
        if cv.waitKey(1) == ESC:
            quit()
