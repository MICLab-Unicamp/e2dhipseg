...
import numpy as np
import cv2 as cv
from utils import ESC, imagePrint, myrotate, get_device, get_slice, normalizeMri, viewnii
...
def train_model(model, criterion, metric, optimizer, scheduler, dataloaders, device, dataset_sizes, 
                ...
                num_epochs=10, save=True, savefolder="tests", patience=0):
    '''
...
def test_model(model, criterion, metric, dataloaders, dataset_sizes, device,
    ...
    for inputs, labels in dataloaders["test"]:
        ...
        with torch.set_grad_enabled(False):
            ...
            if filter_components:
                ...
                npinput, npoutput, ref = inputs[0, 0, :, :, :].cpu().numpy(), outputs[0, 0, :, :, :].detach().cpu().numpy(), labels[0, 0, :, :, :].cpu().numpy() 
                if display_sample:
                    pre_acc = metric(outputs, labels)
                ...
                acc = metric(torch.from_numpy(larg_comp).to(device), labels)
                if display_sample: 
                    viewnii(npinput, larg_comp, ref=ref, id="DICE: {}".format(acc), wait=1)
            else:
                acc = metric(outputs, labels)
    ...
    with open(os.path.join(save_path, str(test_acc) + str(db) + "_test.txt"), 'a') as f:
...
def outsider_test(path, name, db, metric):
    ...
    valid_dbs = ["adni", "mnihip", "mniadni"]
    assert (name in valid_names) and (db in valid_dbs), "{} name or ".format(name)

    accs = []
    if db == "adni":
        db = ADNI(mnireg=False)
        volids = FloatHippocampusDataset(h5path=default_adni, return_volume=True, mode="test", adni=True, data_split=(0.0, 0.0, 1), mnireg=False).get_volids()
    elif db == "mniadni":
        db = ADNI(mnireg=True)
        volids = FloatHippocampusDataset(h5path=default_adni, return_volume=True, mode="test", adni=True, data_split=(0.0, 0.0, 1), mnireg=True).get_volids()
    elif db == "mnihip":
...
if __name__ == "__main__":
    ...
    if arg == "adni":
        ...
        outsider_test("/home/diedre/bigdata/manual_selection_rotated/mnihippodeep", "hippodeep", "mniadni", DICEMetric(apply_sigmoid=False))
    elif arg == "mnihip":
        outsider_test("/home/diedre/bigdata/mni_hip_data/hippodeep", "hippodeep", "mnihip", DICEMetric(apply_sigmoid=False))
    else:
